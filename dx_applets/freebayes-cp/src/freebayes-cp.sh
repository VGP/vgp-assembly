...
main() {
    ...
    if ! [ -e vcf ]; then
    	mkdir vcf
        cat contigs_sorted.bed | awk -v bam=aln.bam -v ref=${ref_name%.gz} '{print "freebayes --bam "bam" --region \""$1":"$2"-"$3"\" --fasta-reference "ref" -g 150 --vcf \"vcf/"$1"_"$2"-"$3".vcf\""}' | parallel --gnu -j $(nproc) -k
    fi
