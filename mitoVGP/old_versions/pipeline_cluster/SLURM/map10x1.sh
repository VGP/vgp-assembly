...
#align
bowtie2 -a -x ${W_URL}/bowtie2_round1/${ABBR} -1 ${W_URL}/${p1[i]} -2 ${W_URL}/${p2[i]} -p ${NPROC} | samtools view -bSF4 - > "${W_URL}/bowtie2_round1/aligned_raw_reads/aligned_${ABBR}_${i}.bam"
...
if ! [[ -e "${W_URL}/freebayes_round1/aligned_${ABBR}_all_sorted.vcf" ]] && ! [[ -e "${W_URL}/freebayes_round1/aligned_${ABBR}_all_sorted.vcf.gz" ]]; then
...
if ! [[ -e "${W_URL}/freebayes_round1/aligned_${ABBR}_all_sorted.vcf.gz" ]]; then

bgzip ${W_URL}/freebayes_round1/aligned_${ABBR}_all_sorted.vcf -@ ${NPROC}

fi

if ! [[ -e "${W_URL}/freebayes_round1/${CONTIG%.*}_10x1.fasta" ]]; then

tabix -p vcf ${W_URL}/freebayes_round1/aligned_${ABBR}_all_sorted.vcf.gz

bcftools consensus ${W_URL}/freebayes_round1/aligned_${ABBR}_all_sorted.vcf.gz -f ${W_URL}/arrow/arrow_round2/${CONTIG} -o ${W_URL}/freebayes_round1/${CONTIG%.*}_10x1.fasta
