...
if [ -z $1 ] || ! [ -e aligned.bam ] ; then
    echo "Usage: ./_docker_qv.sh <sample_id>"
    echo "    requires 'aligned.bam' and 'aligned.bam.bai' in current directory"
    exit -1
fi
...
if [[ -z $1 ]] || [[ ! -e input.fofn ]] ; then
        echo "Usage: ./_submit_qv.sh <ref.fasta>"
