...
testfile_path = "./testfiles/bTaeGut2_trio.hap1.partialscaffold266.blast.tsv"

testdf = readfile(testfile_path, dfname = "testdef1")

actual_read = testdf.iloc[-1].loc['qlen']
expected_read = str(33195)
...
highCovReports = (pd.DataFrame(highCovReportLines[0], columns=['Accession_num', 'Scaffold','Scaffold_len','Scaffold_align_cov','Perc_align_cov'])).sort_values('Perc_align_cov', ascending = False)
## just pass a file name for now - can rig something better later. 
highCovReports.to_csv(args.covreport, sep="\t")

[print (scaff) for scaff in highCovReportLines[1]] 